...
def load_checkpoint(
    ...
    if format in ["inria", "gsplat"]:
        ...
        # Auto-detect common checkpoint layouts to avoid hard failures when the
        # user passes a mismatched `format`.
        is_inria_like = isinstance(model, (tuple, list))
        is_gsplat_like = isinstance(model, dict) and "splats" in model

        if format == "gsplat" and is_inria_like and not is_gsplat_like:
            warnings.warn(
                "Checkpoint appears to be INRIA-style (tuple). Overriding format='gsplat' -> 'inria'.",
                RuntimeWarning,
            )
            format = "inria"
        elif format == "inria" and is_gsplat_like and not is_inria_like:
            warnings.warn(
                "Checkpoint appears to be gsplat-style (dict with 'splats'). Overriding format='inria' -> 'gsplat'.",
                RuntimeWarning,
            )
            format = "gsplat"
